...
select pokemons.name as name, types.name as primary_type
...
select t.name as primary_type, count(p.id) as num_of_pokemon
from pokemons p join types t on p.primary_type = t.id group by t.name;
...
select trainerID, count(pokemon_id) num_pokemon
from pokemon_trainer
where pokelevel = 100
group by trainerID
order by count(pokemon_id);
